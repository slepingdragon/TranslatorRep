...
| `1-11` through `1-13` (pairing arc) | Story 1.10 ✅ → cascade | **1-11 paired persists — must add the `/pairs`-membership listener (partner-side discovery) + Room mirror + restart routing into the Paired home; reads the `/users/{uid}.pairId` 1.10 wrote.** · 1-12 X25519 identity · 1-13 settings+unpair. Real Paired home + Call button = Story 2.2. |
