...
| `1-13` (settings + unpair) | Story 1.11/1.12 ✅ → **last pairing-arc story** | `ModalBottomSheet` (UX-DR35) + two-tap-confirm Unpair → delete `/pairs/{pairId}` + clear Room mirror; partner's `/pairs` listener fires → their status flips to Unpaired (1.11 reconcile already handles it). Completes Epic 1's pairing arc. Real Paired home + Call button = Story 2.2. |
...
| 4 | Pairing arc — persistence + identity | 1.11 **DONE**, 1.12 **DONE** (2026-05-24), 1.13 | Session 3 |
