...
| `1-12` (X25519 identity) · `1-13` (settings + unpair) | Story 1.11 ✅ → cascade | 1-12 generate X25519 keypair (EncryptedSharedPreferences via `security-crypto`) + publish `/users/{uid}/identityPub` (own doc — rules OK) · 1-13 settings sheet + unpair (delete `/pairs/{pairId}` + clear Room mirror; partner's listener fires → Unpaired). Real Paired home + Call button = Story 2.2. |
...
| 4 | Pairing arc — persistence + identity | 1.11 **DONE** (2026-05-24), 1.12, 1.13 | Session 3 |
