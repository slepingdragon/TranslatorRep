...
| `1-3-oracle-vm-livekit-...` | Requires Oracle Cloud account + Cloudflare Registrar + `xaeryx.com` domain (~$10/yr) + SSH keypair | **Story file + Phase 0 runbook landed 2026-05-24** ([`docs/runbooks/oracle-vm-setup.md`](runbooks/oracle-vm-setup.md)). `ready-for-dev`. Bania runs Phase 0 (~2hrs browser/SSH) async; fresh session does Phase 1 (`infra/` Docker Compose + auth-proxy TypeScript) + Phase 2 (deploy + verify) afterward. Unblocks all of Epic 2 (audio calling). |
