...
- [x] **1b.6** `src/routes/token.ts` (POST /v1/token with full middleware chain: parse → auth → rate-limit → paired → mint) + `src/routes/healthz.ts` (GET /v1/healthz with uptime).
- [x] **1b.7** `src/lib/livekit.ts` (AccessToken wrapper, 60s TTL, callType in metadata) + `src/lib/roomName.ts` (deterministic sha256 + base32, 32-char total budget = 5-char prefix + 27-char hash).
- [x] **1b.8** `Dockerfile` — multi-stage: Node 22 bookworm-slim build → distroless/nodejs22-debian12 runtime. Distroless has no shell — exec-form CMD so SIGTERM reaches Node directly.
- [x] **1b.9** `test/` — 4 vitest spec files (roomName, rateLimit, livekit, paired) covering 23 unit tests. All green. `test/setup.ts` stubs required env vars before any test file imports (the logger module eagerly calls getEnv() at top-level — touching any source module would otherwise crash). Integration-style `token.spec.ts` deferred — covered by Phase 2 manual smoke test instead (would need extensive Admin SDK + LiveKit SDK mocking just to re-test the wiring already validated by individual unit tests).
...
- 2026-05-24 (even later, autonomous run while Bania did Oracle Phase 0) — **Phase 1b landed.** `infra/auth-proxy/` is now a fully-implemented Express + TypeScript service per `shared/auth-proxy-api.md`: 10 source files (package + tsconfig + vitest config + Dockerfile + .dockerignore + env + firebaseAdmin + types + lib/{roomName,livekit,logger} + middleware/{auth,paired,rateLimit,errorHandler} + routes/{token,healthz} + index.ts) + 4 unit test files (roomName, rateLimit, livekit, paired = 23 tests, all green). Token mint chain: zod parse → Firebase ID + App Check verify (parallel via Promise.allSettled, ~50-100ms saved per request) → per-UID rate limit (10/min sliding window, in-memory) → Firestore /pairs/{pairId} membership check (tries both memberA/B orderings) → LiveKit JWT mint (60s TTL, callType in metadata). Privacy-safe access log: SHA-256(uid) hashes only, never raw UIDs/tokens. Dockerfile is multi-stage Node 22 → distroless runtime. Local validate green: `npm run typecheck`, `npm test` (23/23), `npm run build` all clean. `package-lock.json` committed (reproducible builds). Status STILL `ready-for-dev` until Phase 2 (deploy + verify on Bania's Oracle VM) — Phase 2 happens after Phase 0 completes; auth-proxy code is ready to deploy via `infra/deploy.sh`.
